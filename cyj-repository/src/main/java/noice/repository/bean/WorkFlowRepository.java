...
public class WorkFlowRepository implements BeanRepository<WorkFlowPo> {
    ...
    @Override
    public String delete(WorkFlowPo po) {
    ...
    @Override
    public int deleteBatch(List<WorkFlowPo> poList) {
        return mapper.deleteByIds(poList.stream().map(WorkFlowPo::getId).collect(Collectors.toList()));
    }
