...
import noice.mapper.auth.bean.RoleMapper;
...
public class RoleRepository implements BeanRepository<RolePo> {
    ...
    @Override
    public String delete(RolePo po) {
    ...
    @Override
    public int deleteBatch(List<RolePo> poList) {
        return mapper.deleteByIds(poList.stream().map(RolePo::getId).collect(Collectors.toList()));
    }
