...
import noice.mapper.create.bean.PersistentMapper;
...
public class PersistentRepository implements BeanRepository<PersistentPo> {
    ...
    @Override
    public String delete(PersistentPo po) {
    ...
    @Override
    public int deleteBatch(List<PersistentPo> poList) {
        return mapper.deleteByIds(poList.stream().map(PersistentPo::getId).collect(Collectors.toList()));
    }
